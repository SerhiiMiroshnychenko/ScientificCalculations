...
for ds_label, ds_dir, linestyle in DATASETS:
    for method_label, fname, color in methods:
        ...
        if len(df) == 0:
            continue
        plt.plot(df['n_features'], df['aucpr'], marker='o', color=color, linestyle=linestyle, label=f'{method_label} ({ds_label})')
...
plt.ylabel('AUC-PR')
plt.title('Порівняння AUC-PR кривих для двох датасетів')
