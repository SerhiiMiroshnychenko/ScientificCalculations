...
def optimize_weights_cv(df, model, target_column='is_successful', n_splits=5):
    ...
    # Використовуємо стратифіковану крос-валідацію для кращих результатів
    ...
    # Цільова функція для scipy оптимізації
    def objective_function(weights):
        # Нормалізуємо ваги
        weights = np.abs(weights)
        if np.sum(weights) == 0:
            return 1.0  # Поганий результат
        ...
        try:
            for train_idx, val_idx in kf.split(X, y):
                ...
                # Обчислюємо F1-score
                fold_score = metrics.f1_score(y_true, y_pred, zero_division=0)
                fold_scores.append(fold_score)

        except Exception:
            return 1.0  # Поганий результат при помилці

        avg_score = np.mean(fold_scores)
        return -avg_score  # Мінімізуємо негативний F1-score

    # Спробуємо scipy оптимізацію
    try:
        logger.info("Використовуємо scipy оптимізацію для пошуку найкращих ваг...")

        # Обмеження: ваги > 0, сума = 1
        constraints = {'type': 'eq', 'fun': lambda x: np.sum(np.abs(x)) - 1}
        bounds = [(0.01, 1.0) for _ in range(len(features))]
        ...
        if result.success and -result.fun > initial_f1:
            best_weights = np.abs(result.x)
            best_weights = best_weights / np.sum(best_weights)
            best_score = -result.fun
            logger.info("Scipy оптимізація успішна!")
        else:
            raise Exception("Scipy не покращила результат")

    except Exception:
        # Fallback на покращений випадковий пошук
        logger.info("Використовуємо покращений випадковий пошук...")

        best_weights = initial_weights.copy()
        best_score = initial_f1

        # Генеруємо більш розумні варіації ваг
        num_combinations = 50

        for i in range(num_combinations):
            if i == 0:
                # Перша ітерація - початкові ваги
                weights = initial_weights.copy()
            else:
                # Генеруємо варіації навколо кращих знайдених ваг
                if i < num_combinations // 2:
                    # Перша половина - варіації навколо початкових ваг
                    base_weights = initial_weights
                else:
                    # Друга половина - варіації навколо поточних найкращих
                    base_weights = best_weights

                # Додаємо контрольований шум
                noise_scale = 0.2 * (1 - i / num_combinations)  # Зменшуємо шум з часом
                noise = np.random.normal(0, noise_scale, len(features))
            ...
            if score > best_score:
                ...
                logger.info(f"Нове покращення на ітерації {i + 1}: F1 = {score:.4f}")

            if (i + 1) % 10 == 0:
                logger.info(f"Перевірено {i + 1}/{num_combinations} комбінацій. Найкращий F1: {best_score:.4f}")
    ...
    improvement = ((best_score - initial_f1) / initial_f1 * 100) if initial_f1 > 0 else 0
    logger.info(f"Покращення: {improvement:.2f}%")
