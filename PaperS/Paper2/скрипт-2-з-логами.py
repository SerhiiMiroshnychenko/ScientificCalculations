...
def main():
    ...
    try:
        ...
        df_tuple = load_data(data_file)

        # Розпакування результату - df має бути першим елементом кортежу
        if isinstance(df_tuple, tuple):
            df = df_tuple[0]  # Припускаємо, що DataFrame є першим елементом кортежу
        else:
            df = df_tuple  # Якщо раптом повертається безпосередньо DataFrame

        # Вибір числових колонок для аналізу (всі окрім цільової)
        numeric_columns = df.select_dtypes(include=['number']).columns.tolist()
        if 'is_successful' in numeric_columns:
            numeric_columns.remove('is_successful')

        logger.info(f"Знайдено {len(numeric_columns)} числових колонок для аналізу")
