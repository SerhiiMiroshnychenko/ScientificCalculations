...
def get_feature_rank(feature, importance_df):
    """
    Повертає ранг ознаки за формулою: Rⱼ(fᵢ)=|{fₗ | Sⱼ(fₗ) ≥ Sⱼ(fᵢ)}|
    """
    feature_value = float(importance_df[importance_df['Ознака'] == feature]['Значущість'])
    return int((importance_df['Значущість'] >= feature_value).sum())
