    ...
    try:
        ...
        for row in reader:
            ...
            if not all(field in row for field in
                       ['order_id', 'customer_id', 'date_order', 'state', 'messages_count']):
...
def _prepare_messages_success_data(csv_path):
    """Prepare data for messages success chart - групую замовлення за кількістю повідомлень"""
    print("Starting _prepare_messages_success_data")
    ...
    try:
        ...
        for row in data:
            try:
                ...
                if messages_count >= 0:  # Виключаємо від'ємні значення
                    is_successful = row['state'] == 'sale'
                    data_points.append((messages_count, is_successful))
        ...
        print(f"Total orders with valid messages count: {total_points}")

        # Визначаємо кількість груп (зменшуємо якщо замовлень мало)
        num_groups = min(20, total_points // 15)  # Мінімум 15 замовлень на групу
        ...
        for i in range(num_groups):
            ...
            min_messages = group_points[0][0]
            max_messages = group_points[-1][0]
            ...
            if min_messages == max_messages:
                range_str = f'{min_messages}'
            else:
                range_str = f'{min_messages}-{max_messages}'
        ...
        print(f"Error preparing messages success data: {str(e)}")
        return None

def _create_messages_success_chart(data):
    """Create chart showing success rate by messages count"""
    if not data:
        return False
    ...
    try:
        ...
        # Менше 50% - світло-синій (#87CEEB)
        # 50-80% - середньо-синій (#4169E1)
        # Більше 80% - темно-синій (#000080)
        colors = []
        ...
        print(f"Error creating messages-success chart: {str(e)}")
        return False

def create_messages_success_chart(csv_path, output_path):
    """
    Creates a scatter plot showing the relationship between messages count and success rate
    ...
    """
    # Prepare data
    data = _prepare_messages_success_data(csv_path)
    ...
    # Create the chart
    success = _create_messages_success_chart(data)
...
if __name__ == '__main__':
    ...
    output_path = 'messages_success_chart'
    create_messages_success_chart(csv_path, output_path)
