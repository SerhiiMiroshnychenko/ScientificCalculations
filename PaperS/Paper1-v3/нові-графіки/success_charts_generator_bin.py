...
def _make_scatter_success_by_grouped_ranges(
    ...
    plt.title(f'{title}\n(each point represents ~{avg_orders} orders)', pad=20, fontsize=14)
    ...
    if len(ranges) <= 10:
        plt.xticks(range(len(ranges)), ranges, rotation=45, ha='right', fontsize=14)
    else:
        plt.xticks(
            range(len(ranges))[::2],
...
def build_order_messages_chart(orders, output_path):
    ...
    _make_scatter_success_by_grouped_ranges(
        title='Success Rate by Messages Count',
...
def build_order_changes_chart(orders, output_path):
    ...
    _make_scatter_success_by_grouped_ranges(
        title='Success Rate by Changes Count',
...
def build_order_amount_chart(orders, output_path):
    ...
    _make_scatter_success_by_grouped_ranges(
        title='Success Rate by Order Amount',
...
def build_discount_total_chart(orders, output_path):
    ...
    ax_right.set_title(f'Success Rate by Discount Amount\n(each point ~{avg_orders} orders)', fontsize=14, pad=12)
    ...
    if len(pos_ranges) <= 10:
        ...
        ax_right.set_xticklabels(pos_ranges, rotation=45, ha='right', fontsize=14)
    else:
        ax_right.set_xticks(range(len(pos_ranges))[::2])
        ax_right.set_xticklabels([pos_ranges[i] for i in range(0, len(pos_ranges), 2)], rotation=45, ha='right', fontsize=14)
...
def build_day_of_week_chart(orders, output_path):
    ...
    plt.title(
        ...
        f'(each point represents ~{avg_orders} orders, point size shows relative number per day)',
        pad=20,
        fontsize=14,
    )
