...
$$ n = \frac{PV}{RT_1} = \frac{101325 \cdot 100}{8,31446261815 \cdot 298,15} = 4087,40\text{ моль} $$
...
1. Переведемо R в кал/(моль·К) для узгодження одиниць вимірювання з коефіцієнтами з таблиці:
   $$ R = 8,31446261815 \text{ Дж/(моль·К)} = 1,986 \text{ кал/(моль·К)} $$
...
2. Розрахуємо коефіцієнти для cv:
   ...
   $$ a_{(v)} = a - R = 10,55 - 1,987 = 8,564 \text{ кал/(моль·К)} $$
   
   Для коефіцієнта при T¹:
   $$ b_{(v)} = b = 2,16 / 10^{3} = 0,00216 \text{ кал/(моль·К²)} $$
   
   Для коефіцієнта при T⁻²:
   $$ c_{(v)} = c = -2,04 / 10^{-5} = -204000 \text{ кал·К/(моль)} $$
...
$$ a_{(v)} = 8,564 \cdot 4,1868 = 35,86 \text{ Дж/(моль·К)} $$
$$ b_{(v)} = 0,00216 \cdot 4,1868 = 0,009044 \text{ Дж/(моль·К²)} $$
$$ c_{(v)} = -204000 \cdot 4,1868 = -854107,2 \text{ Дж·К/(моль)} $$

Тепер теплоємність при сталому об'ємі виражається формулою:
$$ c_v = 35,85 + 0,009044 \cdot T - 854107,2 \cdot T^{-2} \text{ Дж/(моль·К)} $$
...
$$ Q = n\int_{T_1}^{T_2} c_v dT = n\int_{T_1}^{T_2} (a_{(v)} + b_{(v)}T + c_{(v)}T^{-2}) dT $$

Розбиваємо інтеграл на три частини для зручності обчислення:

1. Інтегрування константи $a_v$:
   $$ \int_{T_1}^{T_2} a_{(v)} dT = a_{(v)}(T_2 - T_1) $$

2. Інтегрування лінійного члена $b_vT$:
   $$ \int_{T_1}^{T_2} b_{(v)}T dT = b_{(v)}\frac{T^2}{2}\bigg|_{T_1}^{T_2} = b_{(v)}\frac{T_2^2 - T_1^2}{2} $$

3. Інтегрування оберненого квадрата $c_vT^{-2}$:
   $$ \int_{T_1}^{T_2} c_{(v)}T^{-2} dT = -c_{(v)}\frac{1}{T}\bigg|_{T_1}^{T_2} = c_{(v)}(-\frac{1}{T_2} + \frac{1}{T_1}) $$

Об'єднуючи всі частини, отримуємо:

$$ Q = n[a_{(v)}(T_2 - T_1) + \frac{b_{(v)}}{2}(T_2^2 - T_1^2) + c_{(v)}(-\frac{1}{T_2} + \frac{1}{T_1})] $$

Підставляємо числові значення:

$$ Q = 4087,40 \cdot [35,86(1273,15 - 298,15) + \frac{9,044 \cdot 10^{-3}}{2}(1273,15^2 - 298,15^2) + (-8,541 \cdot 10^{5})(-\frac{1}{1273,15} + \frac{1}{298,15})] $$

$$ Q = 4087,40 \cdot [35,86 \cdot 975 + \frac{0,009044}{2}(1620910,92 - 88893,42) + (-854107,2)(-0,000785 + 0,003354)] $$

$$ Q = 4087,40 \cdot [34953,75 + 0,00452 \cdot 1532017,5 + (-854107,2 \cdot 0,002569] $$

$$ Q = 4087,40 \cdot [34959,87 + 6927,39 - 2193.83] $$

$$ Q = 4087,40 \cdot 39693,43 = 162243115,62 \text{ Дж} = 162243,12 \text{ кДж} $$

### Відповідь
Теплота, що поглинає діоксид вуглецю при нагріванні 100 м³ від 25 до 1000 °С за сталого об'єму, становить Q ≈ 162243,12 кДж.

### Фізична інтерпретація результату
Отримане значення теплоти (162243,12 кДж) показує, скільки теплової енергії необхідно надати 100 м³ діоксиду вуглецю, щоб нагріти його від 25°C до 1000°C при сталому об'ємі. Ця енергія витрачається на збільшення внутрішньої енергії газу, що проявляється у збільшенні кінетичної енергії молекул CO₂ (зростає швидкість їх поступального, обертального та коливального рухів).

Для порівняння, ця кількість теплоти еквівалентна енергії, що виділяється при спалюванні приблизно 3 кг природного газу.
...
# Коефіцієнт при T¹ для cv в Дж/(моль·К²)
b_v *= CAL_TO_JOULE  # b_v = 0.00216 * 4.1868 = 9.044·10⁻³ Дж/(моль·К²)
...
$$ Q = n\int_{T_1}^{T_2} (a_{(v)} + b_{(v)}T + c_{(v)}T^{-2}) \, dT $$

Розбиваємо інтеграл на три частини:

$$ Q = n[\int_{T_1}^{T_2} a_{(v)} \, dT + \int_{T_1}^{T_2} b_{(v)}T \, dT + \int_{T_1}^{T_2} c_{(v)}T^{-2} \, dT] $$
...
$$ Q = n[a_{(v)}(T_2 - T_1) + b_{(v)}\frac{T_2^2 - T_1^2}{2} + c_{(v)}(-\frac{1}{T_2} + \frac{1}{T_1})] $$
...
```
       ...
       Аналітичний метод: 162243.12 кДж
...
$$ \int_{T_1}^{T_2} f(T) \, dT \approx \frac{h}{2}[f(T_1) + 2\sum_{i=1}^{n-1}f(T_i) + f(T_n)] $$
...
```
    ...
    3. Результати розрахунку теплоти:
    	Аналітичний метод: 162243.12 кДж
    	Числовий метод: 162243.09 кДж
    	Різниця між методами: 0.0206 кДж
    	Відносна похибка: 0.000013%
    

#### Інтерпретація результатів:
- Майже ідеальне співпадіння результатів (різниця менше 0.1 кДж)
...
У роботі було використано два методи розрахунку теплоти:
...
2. **Фізичний зміст результату** — отримане значення теплоти (162243.12 кДж) показує, що для нагрівання 100 м³ діоксиду вуглецю від 25°C до 1000°C при сталому об'ємі потрібна значна кількість енергії, еквівалентна спалюванню приблизно 3 кг природного газу. Це підкреслює важливість ефективного використання теплової енергії в металургійних процесах.
