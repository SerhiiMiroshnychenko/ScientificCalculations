...
def main():
    ...
    labels_in = [
        "Q_чав", "Q_м.ш", "Q_дом", "Q_Fe", "Q_шл.утв"
    ]
    values_in = [
        Q_chav_val, Q_msh_val, Q_dom_val, Q_Fe_val, Q_shl_utv_val
    ]
    labels_out = [
        "Q_ст", "Q_шл", "Q_пил", "Q_в.к.в", "Q_втр"
    ]
    values_out = [
    ...
    ax.bar(labels_in, values_in, color='tab:blue', label='Прихід тепла')
    ax.bar(labels_out, values_out, color='tab:orange', label='Витрата тепла', bottom=[0]*len(values_out))
    ax.set_ylabel('кДж')
    ax.set_title('Тепловий баланс: прихід та витрата тепла')
    ax.legend(['Прихід тепла', 'Витрата тепла'])
    plt.tight_layout()
    plt.show()
