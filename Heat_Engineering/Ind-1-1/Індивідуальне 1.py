...
def print_variant_results(results):
    ...
    if results['P_wat'] < results['P_wat_min']:
        print(
            f"   УВАГА! Недостатній тиск води. Потрібно збільшити на {(results['P_wat_min'] - results['P_wat']):.2f} МПа")
    ...
    print(
        f"   3) Температура зовнішньої поверхні: {results['t2']:.1f} °С {'≤' if results['check_t2'] else '>'} {results['t2_max']} °С")
    print(f"   4) Довжина змійовика: {results['L']:.1f} м {'в межах' if results['check_L'] else 'поза межами'} 10-30 м")
    print(
        f"   5) Тиск води: {results['P_wat']} МПа {'≥' if results['check_P_wat'] else '<'} {results['P_wat_min']:.2f} МПа")
