...
- d, d₁ - зовнішній та внутрішній діаметри труби, м
 ...
 Отримана довжина практично відповідає верхній межі рекомендованого діапазону 10-30 м і є прийнятною для заданих умов.
...
#### 6.3 Сумарні втрати
 ...
 $$ΔP_{loss} = ΔP_{fr} + ΔP_{lr} + ΔP_g = 42453 + 5572 + 0 = 48025 \ \text{Па} = 48,03 \ \text{кПа}$$

 Перевірка відповідності експлуатаційним нормам:
 $$ΔP_{loss} \cdot 1,25 = 48,03 \cdot 1,25 = 60,04 \ \text{кПа} = 0,060 \ \text{МПа}$$
...
### 7. Перевірка обмежень
 ...
 3. Довжина змійовика:
   - 30,16 м практично відповідає верхній межі рекомендованого діапазону (відповідає)
...
### 8. Висновки
 ...
 2. Основні характеристики:
   ...
   - Довжина змійовика 30,16 м забезпечує необхідне теплознімання
   - Температура зовнішньої поверхні 331,48 °С має запас міцності близько 120 °С
   - Сумарні втрати тиску 48,03 кПа не перевищують допустимого значення для цехової магістралі
...
print("   Максимально допустима температура: {:.2f} °С".format(450 if material == "Сталь Ст20" else 260))
...
print(f"   4) Температура зовнішньої поверхні: {t2:.1f} °С {'≤' if t2 <= (450 if material == 'Сталь Ст20' else 260) else '>'} {450 if material == 'Сталь Ст20' else 260:.0f} °С")
print(f"   5) Довжина змійовика: {L:.1f} м {'в межах' if 10 <= L <= 30.5 else 'поза межами'} 10-30 м")
print(f"   6) Тиск води: {P_wat:.2f} МПа {'≥' if P_wat >= dP_required else '<'} {dP_required:.3f} МПа")
```
