...
im = ax2.imshow(T, cmap='jet', interpolation='nearest', vmin=35, vmax=120)
...
def update(frame):
    ...
    im = ax2.imshow(temperature_history[frame], cmap='jet', interpolation='nearest', vmin=35, vmax=120)
    ...
    for i in range(temperature_history[frame].shape[0]):
        for j in range(temperature_history[frame].shape[1]):
            ...
            ax2.text(j, i, f'{temp:.1f}°C',
                     ha='center', va='center',
                     color='white' if temp > 80 else 'black',
                     fontsize=8)
...
1. Метод скінченних різниць успішно моделює стаціонарний розподіл
   температури навіть при значних перепадах температури на границях.
