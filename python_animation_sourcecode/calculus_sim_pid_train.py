...
trials=3
incl_angle=np.pi/6*0 # Keep the angle between 0 and +pi/6 radians
g=10
mass_cart=100 # [kg]
...
len_t=len(t)-1
frame_amount=int(t_end/dt)*trials_global
def update_plot(num):
    ...
    cube.set_data([pos_x_cube[int(num/len_t)][num-int(num/len_t)*len_t]-1,pos_x_cube[int(num/len_t)][num-int(num/len_t)*len_t]+1],
    [pos_y_cube[int(num/len_t)][num-int(num/len_t)*len_t],pos_y_cube[int(num/len_t)][num-int(num/len_t)*len_t]])
