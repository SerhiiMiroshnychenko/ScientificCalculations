...
def plot_histograms(data_before, data_after, column_name):
    ...
    plt.figure(figsize=(10, 6))
    sns.histplot(data_before, kde=True, color='blue', alpha=0.5, label='Початкові дані')
    sns.histplot(data_after, kde=True, color='green', alpha=0.5, label='Очищені дані')
    plt.title(f'{column_name} (порівняння гістограм)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
def plot_scatter_before_after(x_before, y_before, x_after, y_after, x_name, y_name):
    ...
    """
    # Діаграма розсіювання до очищення
    plt.figure(figsize=(10, 6))
    plt.scatter(x_before, y_before, alpha=0.7, color='blue')
    plt.title('Дані до видалення викидів')
    plt.xlabel(x_name)
    plt.ylabel(y_name)
    plt.grid(True)
    plt.tight_layout()
    ...
    # Діаграма розсіювання після очищення
    plt.figure(figsize=(10, 6))
    plt.scatter(x_after, y_after, alpha=0.7, color='green')
    plt.title('Дані після видалення викидів')
    plt.xlabel(x_name)
    plt.ylabel(y_name)
    plt.grid(True)
    plt.tight_layout()
...
def plot_regression_results(x, y, x_name, y_name, metrics):
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(x, y, alpha=0.7, color='blue', label='Дані')
    plt.plot(x, y_pred, color='red', label=f'y = {a0:.4f} + {a1:.4f}x')
    plt.title('Діаграма розсіювання з лінією регресії')
    plt.xlabel(x_name)
    plt.ylabel(y_name)
    plt.grid(True)
    plt.legend()
    ...
    # Графік 5: Залишки vs Незалежна змінна
    plt.figure(figsize=(10, 6))
    plt.scatter(x, residuals, alpha=0.7, color='blue')
    plt.axhline(y=0, color='r', linestyle='-')
    plt.title(f'Залишки vs {x_name}')
    plt.xlabel(x_name)
    plt.ylabel('Залишки')
    plt.grid(True)
    plt.tight_layout()
