...
class Odoo18DeveloperScraper:
    def __init__(self, start_url, delay=0.8, retry=2, timeout=30):
        ...
        self.base_path_prefix = '/documentation/18.0/developer'
        self.visited = set()
        self.ordered_urls = []
        self.doc = Document()
    ...
    def normalize_url(self, url, current_url=None):
        ...
        - тільки /documentation/18.0/developer*
    ...
    def is_valid_url(self, url):
        """Перевіряє, що URL внутрішній, у межах гілки developer і не статичний."""
        if not url:
            return False
        parsed = urlparse(url)
    ...
    def _process_element(self, element, text_list, parent_tag=None):
        ...
        if element.name == 'li':
            ...
            # ВАЖЛИВО: не виходимо рано — у li можуть бути вкладені <pre>/<code>/таблиці
        ...
        if element.name in ['pre', 'code']:
            ...
            try:
                ...
                if parent and getattr(parent, 'find', None):
                    cap_div = None
                    # якщо pre безпосередньо всередині 'literal-block-wrapper'
                    ...
                    # або сусідній попередній елемент
                    if not cap_div:
    ...
    def save(self, filename='odoo_18_developer_docs.docx'):
...
def main():
    start_url = "https://www.odoo.com/documentation/18.0/developer.html"
    scraper = Odoo18DeveloperScraper(start_url, delay=0.8, retry=2, timeout=30)
    scraper.scrape()
    scraper.save('odoo_18_developer_docs.docx')
