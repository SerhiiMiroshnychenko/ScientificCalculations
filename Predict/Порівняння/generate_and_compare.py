...
def compare_generated_vs_target(gen_df, target_df):
    ...
    d1 = gen_df[gen_df['order_id'].isin(common_ids)].sort_values('order_id').reset_index(drop=True)
    d2 = target_df[target_df['order_id'].isin(common_ids)].sort_values('order_id').reset_index(drop=True)
    ...
    print(f"Unique to Generated: {len(cols1 - cols2)}")
    print(f"Unique to Target: {len(cols2 - cols1)}")
    ...
    for col in common_cols:
        ...
        if match_pct < 100:
            ...
            if match_pct < 99.9: # Only show if significant difference
                 diff_mask = ~np.isclose(s1.fillna(-9999), s2.fillna(-9999), rtol=1e-05, atol=1e-08) if pd.api.types.is_numeric_dtype(s1) and pd.api.types.is_numeric_dtype(s2) else (s1.astype(str) != s2.astype(str))
                 diffs = pd.DataFrame({'ID': d1.loc[diff_mask, 'order_id'], 'Gen': s1[diff_mask], 'Tgt': s2[diff_mask]})
